...
let tls_secure_example () =
  ...
  let tls_config = {
    ...
    ca_cert_file = None;  (* Use system trust store *)
  ...
  let client = Client.create 
    ~host:"secure-proton-server.example.com" 
    ~port:9440  (* TLS port *)
    ~tls_config 
...
let mtls_example () =
  ...
  let client = Client.create 
    ~host:"mtls-proton-server.example.com"
    ~port:9440
  ...
  Printf.printf "Custom CA: %s\n" 
    (match client.conn.tls_config.ca_cert_file with 
     | Some path -> path 
     | None -> "System default");
  
  Printf.printf "Client cert: %s\n"
    (match client.conn.tls_config.client_cert_file with
     | Some path -> path
     | None -> "None");
...
let insecure_dev_example () =
  ...
  let dev_tls_config = {
    ...
    insecure_skip_verify = true;  (* WARNING: Only for development! *)
  } in
  
  let client = Client.create
    ~host:"localhost"
    ~port:9440
    ~tls_config:dev_tls_config
    ()
  in
