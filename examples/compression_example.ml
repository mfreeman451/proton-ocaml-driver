...
let () =
  ...
  let* () = Lwt_io.printf "   LZ4 method byte:  0x%02x\n" (Compress.method_to_byte Compress.LZ4) in
  let* () = Lwt_io.printf "   ZSTD method byte: 0x%02x\n" (Compress.method_to_byte Compress.ZSTD) in
  ...
  let* () = Lwt_io.printf "   Original size: %d bytes\n" (Bytes.length original) in
