...
let rec compile_reader s : (in_channel -> int -> value array) =
  ...
  match Column_primitives.reader_primitive_of_spec_normalized s with
  | Some r -> r
  | None -> (match Column_datetime.reader_datetime_of_spec_normalized s with
             | Some r -> r
             | None -> (match Column_complex.reader_complex_of_spec_normalized ~resolver:reader_of_spec s with
                        | Some r -> r
                        | None -> (match Column_lowcardinality.reader_lowcardinality_of_spec_normalized ~resolver:reader_of_spec s with
                                   | Some r -> r
...
let rec compile_reader_br s : (Buffered_reader.t -> int -> value array) =
  (* [s] is already normalized *)
  match Column_primitives.reader_primitive_of_spec_br_normalized s with
  | Some r -> r
  | None -> (match Column_datetime.reader_datetime_of_spec_br_normalized s with
             | Some r -> r
             | None -> (match Column_complex.reader_complex_of_spec_br_normalized ~resolver:reader_of_spec_br s with
                        | Some r -> r
                        | None -> (match Column_lowcardinality.reader_lowcardinality_of_spec_br_normalized ~resolver:reader_of_spec_br s with
                                   | Some r -> r
                                   | None -> failwith (Printf.sprintf "Unsupported column type: %s" s))))
