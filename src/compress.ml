  ...
  | LZ4 | ZSTD ->
      ...
      let rec loop off =
        if off >= total_len then Lwt.return_unit
          ...
          let method_byte = match method_ with LZ4 -> method_to_byte LZ4 | ZSTD -> method_to_byte ZSTD | None -> assert false in
          (* header inside frame *)
          Bytes.set frame checksum_size (Char.chr method_byte);
          Binary.bytes_set_int32_le frame (checksum_size + 1) (Int32.of_int (compressed_size + compress_header_size));
          ...
          let h_cxx = Cityhash.cityhash128_sub frame checksum_size (compress_header_size + compressed_size) in
          ...
          let dbg = match Sys.getenv_opt "PROTON_DEBUG" with Some ("1"|"true"|"TRUE"|"yes"|"YES") -> true | _ -> false in
          if dbg then (
            let hex s =
            ...
            printf "[compress] frame(method=0x%02x) comp_size=%d uncomp=%d checksum=%s alt_sizes_first=%s use=%s\n%!"
              method_byte (compressed_size + compress_header_size) chunk_len (hex checksum) (hex c_sizes_first) "cxx";
            ...
            let payload0 =
              let take = min 32 compressed_size in Bytes.sub frame (checksum_size + compress_header_size) take in
            printf "[compress] hashed_hdr=%s payload0=%s hashed_len=%d\n%!"
              (hex hdr) (hex payload0) (compress_header_size + compressed_size)
            ;
            ...
            (match Sys.getenv_opt "PROTON_DUMP_HASH" with
             | Some ("1"|"true"|"TRUE"|"yes"|"YES") ->
                ...
                Bytes.blit frame (checksum_size + compress_header_size) to_hash compress_header_size compressed_size;
                (try if not (Sys.file_exists "debug") then U.mkdir "debug" 0o755 with _ -> ());
                let ts = int_of_float (U.gettimeofday ()) in
                let base = sprintf "debug/hashed_m%02x_c%d_u%d_t%d" method_byte (compressed_size + compress_header_size) chunk_len ts in
                let binf = base ^ ".bin" in
                let hexf = base ^ ".hex" in
                let metaf = base ^ ".meta" in
                let ocb = open_out_bin binf in
                output_bytes ocb to_hash; close_out ocb;
                ...
                fprintf ocm "method=0x%02x\ncomp_size=%d\nuncomp=%d\nhash_len=%d\nuse=%s\nchecksum=%s\n"
                  method_byte (compressed_size + compress_header_size) chunk_len to_hash_len "cxx" (hex checksum);
                close_out ocm
             | _ -> ())
          );
          let dbg = match Sys.getenv_opt "PROTON_DEBUG" with Some ("1"|"true"|"TRUE"|"yes"|"YES") -> true | _ -> false in
          if dbg then (
            ...
            printf "[compress] frame(method=0x%02x) comp_size=%d uncomp=%d checksum=%s\n%!"
              method_byte (compressed_size + compress_header_size) chunk_len (hex checksum)
          );
...
module Stream = struct
  ...
  let create writev method_ =
    { method_; writev; buf = Bytes.create max_block_size; off = 0 }
  ...
  let[@inline] write_frame (t:t) (chunk:bytes) (len:int) : unit Lwt.t =
    ...
    | LZ4 | ZSTD ->
        let to_compress =
          if len = Bytes.length chunk then chunk else Bytes.sub chunk 0 len
        in
        let compressed = match t.method_ with
        ...
        let total = checksum_size + compress_header_size + compressed_size in
        let frame = Bytes.create total in
        let method_byte = match t.method_ with
        ...
        if dbg then (
          ...
          let c_sizes_first = Cityhash.to_bytes h_alt in
          printf "[compress] frame(method=0x%02x) comp_size=%d uncomp=%d checksum=%s alt_sizes_first=%s use=%s\n%!"
            method_byte (compressed_size + compress_header_size) len (hex checksum) (hex c_sizes_first) "cxx";
          let hdr = Bytes.sub frame checksum_size compress_header_size in
          let payload0 = let take = min 32 compressed_size in Bytes.sub frame (checksum_size + compress_header_size) take in
          printf "[compress] hashed_hdr=%s payload0=%s hashed_len=%d\n%!"
            (hex hdr) (hex payload0) (compress_header_size + compressed_size)
          ;
          (match Sys.getenv_opt "PROTON_DUMP_HASH" with
           | Some ("1"|"true"|"TRUE"|"yes"|"YES") ->
              ...
              (try if not (Sys.file_exists "debug") then U.mkdir "debug" 0o755 with _ -> ());
              let ts = int_of_float (U.gettimeofday ()) in
              let base = sprintf "debug/hashed_m%02x_c%d_u%d_t%d" method_byte (compressed_size + compress_header_size) len ts in
              ...
              fprintf ocm "method=0x%02x\ncomp_size=%d\nuncomp=%d\nhash_len=%d\nuse=%s\nchecksum=%s\n"
                method_byte (compressed_size + compress_header_size) len to_hash_len "cxx" (hex checksum);
              close_out ocm
           | _ -> ())
        );
        let dbg = match Sys.getenv_opt "PROTON_DEBUG" with Some ("1"|"true"|"TRUE"|"yes"|"YES") -> true | _ -> false in
        if dbg then (
          ...
          in
          printf "[compress] frame(method=0x%02x) comp_size=%d uncomp=%d checksum=%s\n%!"
            method_byte (compressed_size + compress_header_size) len (hex checksum)
        );
        Bytes.blit checksum 0 frame 0 checksum_size;
        let dbg = match Sys.getenv_opt "PROTON_DEBUG" with Some ("1"|"true"|"TRUE"|"yes"|"YES") -> true | _ -> false in
        if dbg then (
  ...
  let write_char (t:t) (c:char) : unit Lwt.t =
    if t.off < max_block_size then (
      Bytes.set t.buf t.off c; t.off <- t.off + 1; Lwt.return_unit
    ) else (
      let* () = write_frame t t.buf max_block_size in
      t.off <- 0;
      Bytes.set t.buf 0 c; t.off <- 1; Lwt.return_unit
    )

  let flush (t:t) : unit Lwt.t =
    if t.off = 0 then Lwt.return_unit else (
      let* () = write_frame t t.buf t.off in
      t.off <- 0; Lwt.return_unit
    )
end
