(** {1 Binary Writer Module}
    ...
    Low-level binary serialization functions for the Proton wire protocol.
    These functions write various data types to buffers in the format expected
    by the Proton server.
...
(** [write_block_info_to_buffer buffer block_info] writes block metadata to the buffer.
    ...
    Serializes block information including checksums and compression details
    as required by the Proton protocol.
...
(** [write_value_to_buffer buffer value] writes a column value to the buffer.
    ...
    Handles all supported column types including integers, floats, strings,
    dates, and arrays. The serialization format depends on the value type.
