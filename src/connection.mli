(** {1 Connection Module}
    ...
    Low-level connection management for Timeplus Proton database. This module handles the network
    connection, TLS configuration, authentication, and packet-level communication with the server.
...
type tls_config = {
  enable_tls : bool;  (** Whether to use TLS for the connection. Default: false *)
  ca_cert_file : string option;  (** Path to CA certificate file for server verification *)
  client_cert_file : string option;  (** Path to client certificate for mutual TLS authentication *)
  client_key_file : string option;  (** Path to client private key for mutual TLS authentication *)
...
(** TLS/SSL configuration for secure connections.
    ...
    Controls how the client establishes secure connections to the server. By default, TLS is
    disabled for compatibility with local development setups.
...
(** Server information received during handshake.
    ...
    Contains metadata about the connected Proton server including version, timezone, and display
    name.
...
(** Packet types received from the server during query execution.
    ...
    Represents different types of data and control packets that can be received from the server
    during query processing.

    @since 1.0.0 *)
type packet =
  | PData of Block.t  (** Data block containing query result rows *)
  | PProgress  (** Progress update for long-running queries *)
  | PProfileInfo  (** Query profiling information *)
  | PTotals of Block.t  (** Aggregated totals for the query *)
  | PExtremes of Block.t  (** Extreme values (min/max) for columns *)
  | PLog of Block.t  (** Server log messages *)
  | PEndOfStream  (** Indicates the end of the result stream *)
...
type t = {
  ...
  tls_config : tls_config;  (** TLS configuration settings *)
  mutable fd : Lwt_unix.file_descr option;  (** Unix file descriptor for the socket connection *)
  mutable tls : Tls_lwt.Unix.t option;  (** TLS session if TLS is enabled *)
  mutable connected : bool;  (** Whether the connection is currently active *)
  mutable srv : server_info option;  (** Server information received during handshake *)
  ctx : Context.t;  (** Connection context with settings and state *)
  compression : Protocol.compression;  (** Compression method to use for data transfer *)
}
(** Connection handle representing an active connection to a Proton server.

    Contains all connection state including network socket, TLS session, server information, and
...
(** Default TLS configuration with TLS disabled.
    ...
    Provides a safe default configuration suitable for local development. For production use, enable
    TLS and configure certificates appropriately.
...
(** [create ?host ?port ?database ?user ?password ?tls_config ?compression ?settings ()] creates a
    ...
    This only creates the connection object; use {!connect} to establish the actual network
    connection.
    ...
    {[
      let conn =
        Connection.create ~host:"proton.example.com" ~port:8463 ~user:"myuser" ~password:"mypass"
...
(** [connect conn] establishes a connection to the server.
    ...
    Performs the network connection, TLS handshake (if configured), and authentication with the
    server.
...
(** [send_query conn ?query_id query] sends a query to the server.
    ...
    Sends a complete query including the data termination marker. Use this for queries that don't
...
(** [send_query_without_data_end conn ?query_id query] sends a query without termination.
    ...
    Sends a query but doesn't send the data termination marker. Use this when you need to send data
    blocks after the query (e.g., for INSERT).
...
(** [send_data_block conn block] sends a data block to the server.
    ...
    Used for sending data during INSERT operations or when the server requests data.
...
(** [receive_packet conn] receives the next packet from the server.
    ...
    Waits for and reads the next packet from the server, which could be data, progress information,
    or control packets.

    @param conn The connection handle
    @return A promise resolving to the received packet
    @raise Errors.Server_exception if the server sends an error
...
(** [send_hello conn] sends the client handshake message.
    ...
    Sends client information including version, database, and credentials to initiate the connection
    handshake.
...
(** [receive_hello conn] receives and processes the server handshake.
    ...
    Receives the server's response to the client hello, extracting server information and
    capabilities.
...
val receive_data_block :
  t -> compressible:bool -> (bytes -> int -> int -> int Lwt.t) -> Block.t Lwt.t
(** [receive_data_block conn ~compressible read_fn] reads a data block.

    Low-level function to read a data block from the connection, handling compression if enabled.
