...
let parse_datetime64_params s =
  if not (String.contains s '(') then (3, None)
    ...
    | [ precision_str; tz_str ] ->
        ...
        let clean_tz =
          if String.length tz >= 2 && tz.[0] = '\'' && tz.[String.length tz - 1] = '\'' then
            String.sub tz 1 (String.length tz - 2)
          else tz
        in
        (precision, Some clean_tz)
    | _ -> (3, None)

let reader_datetime_of_spec (s : string) : (in_channel -> int -> value array) option =
      ...
      Some
        (fun ic n ->
          ...
          for i = 0 to n - 1 do
            ...
            a.(i) <- DateTime64 (value, precision, timezone)
          done;
          a)
  | _ when has_prefix s "datetime" ->
      let timezone =
        match parse_datetime_params s with
        | None -> None
        | Some tz ->
            Some
              (if tz.[0] = '\'' && tz.[String.length tz - 1] = '\'' then
                 String.sub tz 1 (String.length tz - 2)
               else tz)
      in
      Some
        (fun ic n ->
          let a = Array.make n Null in
          for i = 0 to n - 1 do
            let ts = read_int32_le ic |> Int64.of_int32 in
            a.(i) <- DateTime (Int64.logand ts 0xFFFFFFFFL, timezone)
          done;
          a)
  | _ -> None

let reader_datetime_of_spec_br (s : string) : (Buffered_reader.t -> int -> value array) option =
  let s = String.lowercase_ascii (String.trim s) in
  match true with
  | _ when s = "date" ->
      Some
  ...
  | _ when has_prefix s "datetime" ->
      ...
      Some
        (fun br n ->
          let a = Array.make n Null in
          for i = 0 to n - 1 do
            let ts = read_int32_le_br br |> Int64.of_int32 in
            a.(i) <- DateTime (Int64.logand ts 0xFFFFFFFFL, timezone)
          done;
          a)
  | _ -> None
