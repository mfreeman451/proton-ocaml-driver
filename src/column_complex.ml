...
let parse_map_types s =
  if not (String.contains s '(') then failwith "Invalid map type"
    ...
    match String.split_on_char ',' types_str with
    | [ k; v ] -> (String.trim k, String.trim v)
    | _ -> failwith "Map must have exactly 2 types"
  ...
  | _ when starts_with ~prefix:"array(" s ->
      ...
      Some
        (fun ic n ->
          ...
          for i = 0 to n - 1 do
            let e = Int64.to_int offsets.(i) in
            res.(i) <- Array (Array.sub all !start_idx (e - !start_idx));
            start_idx := e
          done;
          res)
      ...
      Some
        (fun ic n ->
          ...
          for i = 0 to n - 1 do
            ...
            let pairs = ref [] in
            while !idx < e do
              pairs := (keys.(!idx), vals.(!idx)) :: !pairs;
              incr idx
            done;
            res.(i) <- Map (List.rev !pairs)
          done;
          res)
  | _ when starts_with ~prefix:"tuple(" s ->
      ...
      Some
        (fun ic n ->
          ...
          for i = 0 to n - 1 do
            res.(i) <- Tuple (List.map (fun a -> a.(i)) cols)
          done;
          res)
  | _ when starts_with ~prefix:"nullable(" s ->
      ...
      Some
        (fun ic n ->
          ...
          for i = 0 to n - 1 do
            nulls.(i) <- read_uint8 ic <> 0
          done;
          let vals = inner_reader ic n in
          for i = 0 to n - 1 do
            if nulls.(i) then vals.(i) <- Null
          done;
          vals)
  | _ -> None

let reader_complex_of_spec_br ~(resolver : string -> Buffered_reader.t -> int -> value array)
  ...
  | _ when starts_with ~prefix:"array(" s ->
      ...
      Some
        (fun br n ->
          let offsets = Array.make n 0L in
          for i = 0 to n - 1 do
            offsets.(i) <- read_uint64_le_br br
          done;
          let total = if n = 0 then 0 else Int64.to_int offsets.(n - 1) in
