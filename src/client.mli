(** {1 Timeplus Proton OCaml Driver Client}
    ...
    This module provides the main client interface for connecting to and interacting with
    Timeplus Proton database servers. It supports query execution, streaming results,
    and efficient bulk inserts.
...
(** Streaming result container that pairs query results with column metadata.
    Used by streaming functions that need to return both data and schema information.
...
(** The main client type representing a connection to a Proton server.
    Contains the underlying connection handle used for all database operations. *)
type t = { conn : Connection.t }

(** [create ?host ?port ?database ?user ?password ?tls_config ?compression ?settings ()]
    creates a new client connection to a Proton server.
    ...
    Example:
    {[
      let client = Client.create ~host:"proton.example.com" ~port:8463
...
(** [execute client query] executes a SQL query and returns all results at once.
    ...
    This function is best suited for queries that return a manageable amount of data.
    For large result sets, consider using the streaming functions instead.
...
(** {2 Streaming Query Functions}
    ...
    These functions provide efficient streaming access to query results,
    allowing processing of large datasets without loading all data into memory. *)

(** [query_fold client query ~init ~f] executes a query and folds over the results.
    
    Processes query results incrementally using a fold function, which is memory-efficient
    ...
    Example:
    {[
      let%lwt sum = Client.query_fold client
        "SELECT value FROM metrics"
        ~init:0.0
        ~f:(fun acc row ->
          match row with
          | [Column.Float v] -> Lwt.return (acc +. v)
          | _ -> Lwt.return acc)
    ]}
...
(** [query_iter client query ~f] executes a query and applies a function to each row.
    ...
    Example:
    {[
      Client.query_iter client "SELECT * FROM logs" 
        ~f:(fun row -> print_row row; Lwt.return_unit)
    ]}
...
(** [query_collect client query] executes a query and collects all results into a list.
    ...
    Convenience function that loads all query results into memory at once.
...
(** {2 Advanced Streaming with Column Metadata}
    ...
    These functions provide access to both query results and column schema information,
    useful when the structure of results needs to be examined dynamically. *)

(** [query_fold_with_columns client query ~init ~f] folds over query results with column metadata.
    
    Similar to [query_fold] but provides column information to the fold function,
    allowing schema-aware processing of results.
...
(** {2 Data Insertion Functions}
    ...
    Functions for inserting data into Proton tables, including support for
    high-performance asynchronous bulk inserts. *)

(** [create_async_inserter ?config client table_name] creates an asynchronous bulk inserter.
    
    Creates an inserter that batches inserts for improved performance.
    The inserter manages its own buffer and flushes data according to the configuration.
...
(** [insert_rows client table_name ?columns rows] performs a synchronous bulk insert.
    ...
    Inserts multiple rows in a single operation. More efficient than inserting rows
    individually for batch operations.
    ...
    {[
      let rows = [
        [Column.String "event1"; Column.Int 42];
        [Column.String "event2"; Column.Int 43];
      ] in
      Client.insert_rows client "events" 
        ~columns:[("name", "String"); ("value", "Int32")] rows
    ]}
...
(** [insert_row client table_name ?columns row] inserts a single row into a table.
    ...
    Convenience function for inserting a single row. For multiple rows,
    use [insert_rows] or [create_async_inserter] for better performance.
