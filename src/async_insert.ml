...
let default_config table_name =
  {
    ...
    (* Increased default batch size for native protocol *)
...
let logf fmt = if env_insert_debug () then Printf.printf fmt else Printf.ifprintf stdout fmt
...
let chunk_rows rows chunk_size =
  let rec loop acc cur n = function
    ...
    | x :: xs ->
        if n = 0 then loop (List.rev (x :: cur) :: acc) [] (chunk_size - 1) xs
        else loop acc (x :: cur) (n - 1) xs
  in
  ...
  if n_rows = 0 then Lwt.return_unit
    ...
    let rec retry_send attempt =
      if attempt > inserter.config.max_retries then
        Lwt.fail_with
          (Printf.sprintf "Failed to insert batch after %d retries" inserter.config.max_retries)
      else
        Lwt.catch
          (fun () ->
            ...
            let insert_sql =
              Printf.sprintf "INSERT INTO %s (%s) FORMAT Native" inserter.config.table_name
                (String.concat ", " names)
            in
            ...
            let send_one_chunk ch =
              ...
              let block_columns =
                List.mapi
                  (fun i (name, type_spec) -> { Block.name; type_spec; data = cols_arrays.(i) })
                  columns
              in
              let n = if Array.length cols_arrays = 0 then 0 else Array.length cols_arrays.(0) in
              let n_columns = List.length columns in
              let data_block =
                { Block.n_columns; n_rows = n; columns = Array.of_list block_columns }
              in
            ...
            let rec drain () =
              let* pkt = Connection.receive_packet inserter.connection in
              match pkt with PEndOfStream -> Lwt.return_unit | _ -> drain ()
            in
          ...
          (fun exn ->
            ...
            | Errors.Server_exception { code; message; nested } ->
                logf "Native insert attempt %d failed: server code=%d msg=%s%s\n%!" attempt code
                  message
                  (match nested with None -> "" | Some s -> "; nested=" ^ s)
            | _ -> logf "Native insert attempt %d failed: %s\n%!" attempt (Printexc.to_string exn));
...
let flush_buffer inserter =
  Lwt_mutex.with_lock inserter.mutex (fun () ->
      if inserter.buffer.row_count = 0 then Lwt.return_unit
      else
        let rows_to_send = List.rev inserter.buffer.rows in
        let columns_spec =
          match (inserter.buffer.column_names, inserter.buffer.column_types) with
...
let rec flush_loop inserter =
  if not inserter.running then Lwt.return_unit
  else
  ...
  Lwt_mutex.with_lock inserter.mutex (fun () ->
      let* () =
        if inserter.buffer.column_names = None then (
          if columns = [] then
            Lwt.fail_with "Column names/types must be provided with the first row"
          else
...
(* Add multiple rows *)
let add_rows ?columns inserter rows = Lwt_list.iter_s (add_row ?columns inserter) rows

(* Force a flush *)
let flush inserter = flush_buffer inserter

(* Get buffer statistics *)
let get_stats inserter =
  Lwt_mutex.with_lock inserter.mutex (fun () ->
