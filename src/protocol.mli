(** {1 Protocol Module}
    ...
    Wire protocol definitions for Timeplus Proton client-server communication.
    This module defines packet types, compression methods, and other protocol-level
    constants used in the binary protocol.
...
(** Client-to-server packet types.
    ...
    Represents the different types of packets that can be sent from
    the client to the server.
    
    @since 1.0.0 *)
type client_packet =
  | Hello
  (** Initial handshake packet with client information *)
...
(** Server-to-client packet types.
    ...
    Represents the different types of packets that can be received from
    the server in response to client requests.
    
    @since 1.0.0 *)
type server_packet =
  | SHello
  (** Server handshake response with server information *)
  
  | SData
  (** Data block containing query results *)
  
  | SException
  (** Error or exception from the server *)
  ...
  (** Response to tables status request *)
...
(** [compression_to_int compression] converts compression method to protocol integer.
    ...
    Maps compression methods to their enable/disable values in the protocol.
    Returns 0 for no compression, 1 for enabled compression (LZ4 or ZSTD).
...
(** Query processing stage indicators.
    ...
    Controls how much processing the server performs on query results
...
type query_processing_stage =
  ...
  | WithMergeableState
  (** Include mergeable state for distributed queries *)
  
  | Complete
  (** Complete query processing (default) *)
