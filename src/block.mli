(** {1 Block Module}
    ...
    Data block representation for Timeplus Proton query results.
    Blocks are the fundamental unit of data transfer between client and server,
    containing both data rows and column metadata.
...
(** Column metadata and data within a block.
    ...
    Represents a single column in a data block, including its name,
    type specification, and the actual data values.
...
type column = {
  ...
  (** Array of values for this column across all rows *)
}

(** Data block containing query results.
    
    A block represents a batch of rows returned from a query,
    with data organized by columns. Each block is self-contained
    with its own column metadata.
...
(** [read_block ~revision ic] reads a data block from an input channel.
    ...
    Reads a complete data block including column metadata and row data
    according to the protocol revision.
...
(** [read_block_br ~revision br] reads a data block using a buffered reader.
    ...
    Similar to {!read_block} but uses a buffered reader for better
    performance with compressed or network streams.
...
(** [get_rows block] extracts all rows from a block as a list of lists.
    ...
    Transforms the column-oriented storage into row-oriented format
    for easier processing. Each inner list represents one row with
    ...
    {[
      ...
      List.iter (fun row ->
        List.iter (fun value ->
          print_string (Column.value_to_string value ^ " ")
        ) row;
        print_newline ()
      ) rows
    ]}
...
(** [columns_with_types block] returns column names and their types.
    ...
    Extracts the schema information from a block as a list of
    (name, type) pairs. Useful for understanding the structure
    of query results.
    ...
    {[
      ...
      List.iter (fun (name, typ) ->
        Printf.printf "Column %s has type %s\n" name typ
      ) columns
    ]}
