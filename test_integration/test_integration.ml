  ...
  Lwt_main.run
    (let open Lwt.Syntax in
     try
       ...
       (* Insert data *)
       let insert =
         "INSERT INTO test_advanced VALUES (\n\
         \        1,\n\
         \        'Product A',\n\
         \        99.99,\n\
         \        now(),\n\
       ...
       let* _ =
         Client.execute client
           "INSERT INTO test_advanced VALUES (2, 'Product B', 149.50, now(), false, ['tag4'])"
       in
       let* _ =
         Client.execute client
           "INSERT INTO test_advanced VALUES (3, 'Product C', 200.00, now(), true, ['tag5', \
            'tag6'])"
       in
       ...
       (* Query with WHERE *)
       let* active =
         Client.execute client "SELECT name, price FROM test_advanced WHERE is_active = true"
       in
...
let test_compression () =
  ...
  Lwt_main.run
    (let open Lwt.Syntax in
     ...
     let client_lz4 = Client.create ~host:"localhost" ~port:8463 ~compression:Compress.LZ4 () in
     ...
     try
       ...
       let client_zstd = Client.create ~host:"localhost" ~port:8463 ~compression:Compress.ZSTD () in
       ...
       let client_none = Client.create ~host:"localhost" ~port:8463 ~compression:Compress.None () in
  ...
  Lwt_main.run
    (let open Lwt.Syntax in
     try
       ...
       let* _ =
         Client.execute client "CREATE TABLE test_stream (id int32, value string) ENGINE = Memory"
       in

       (* Insert test data *)
       let* _ =
         Client.execute client "INSERT INTO test_stream VALUES (1, 'one'), (2, 'two'), (3, 'three')"
       in

       (* Test fold *)
       Printf.printf "Testing fold...\n";
       let* sum =
         Client.query_fold client "SELECT id FROM test_stream" ~init:0 ~f:(fun acc row ->
             match row with
       ...
       let* () =
         Client.query_iter client "SELECT value FROM test_stream" ~f:(fun row ->
             match row with
             | Column.String s :: _ ->
