...
module TableFormatter = struct
  let get_column_widths (result : TestResult.t) =
    ...
    let data_widths =
      List.fold_left
        (fun acc row ->
          List.map2 (fun w v -> max w (String.length (Column.value_to_string v))) acc row)
  ...
  let format_row values widths =
    let cells =
      List.map2
        (fun v w ->
          let s = match v with `String s -> s | `Value v -> Column.value_to_string v in
  ...
  let format_result (result : TestResult.t) =
    if result.rows = [] then sprintf "Empty result set (0 rows)\n"
    else
...
module DataTypeTests = struct
  ...
  let insert_test_data client table_name =
    ...
    let one id name age balance bignum =
      ...
      sprintf
        "INSERT INTO %s (id, name, age, balance, bignum, created) SELECT to_int32(%d), '%s', \
         to_uint32(%d), to_float64(%f), to_int64(%Ld), now()"
  ...
  let test_bounded_query conn table_name column_filter =
    ...
    let bounded_query =
      sprintf "SELECT %s FROM %s ORDER BY id LIMIT 5 SETTINGS query_mode='table'" column_filter
        table_name
    in
    ...
    let rec collect_results acc_rows col_info =
      ...
      | Connection.PEndOfStream ->
          ...
          let names, types = match col_info with Some (n, t) -> (n, t) | None -> ([], []) in
          Lwt.return
            {
    ...
    let* bounded_result =
      Lwt.catch
        (fun () -> Lwt_unix.with_timeout 3.0 (fun () -> collect_results [] None))
        (fun ex ->
    ...
    if bounded_result.rows = [] then (
      ...
      let* () =
        let insert_one id =
          let insert =
            sprintf
              "INSERT INTO %s (id, name, age, balance, bignum, created) SELECT to_int32(%d), \
               'Stream%d', to_uint32(%d), to_float64(%f), to_int64(%Ld), now()"
              table_name id id (20 + id)
              (1000.0 +. float_of_int id)
              (Int64.of_int (1000000 + id))
          in
          ...
          Lwt.catch
            (fun () ->
              ...
              | Connection.PData block ->
                  ...
                  if num_rows > 0 then printf "[DEBUG] Streaming received %d rows\n%!" num_rows
                  else ();
                  let col_info =
                    match col_info with
                  ...
                  collect_streaming (acc_rows @ rows) col_info (row_count + num_rows) new_attempts
                    max_attempts
              | Connection.PEndOfStream ->
                  printf "[DEBUG] Streaming received EndOfStream\n%!";
                  let elapsed_ms = (Unix.gettimeofday () -. start_time) *. 1000.0 in
                  let names, types =
                    match col_info with Some (n, t) -> (n, t) | None -> ([], [])
                  in
                  let* () = Connection.disconnect conn2 in
                  Lwt.return
                    {
    ...
    else Lwt.return bounded_result
        ...
        Lwt.catch
          (fun () ->
            ...
            | Connection.PData block ->
                let rows = Block.get_rows block in
                let num_rows = List.length rows in
                if num_rows > 0 then printf "[DEBUG] Unbounded: received %d rows\n%!" num_rows;
                let col_info =
                  match col_info with
                  | None when num_rows > 0 ->
                      let cols = Block.columns_with_types block in
...
let run_live_tests () =
  ...
  let settings =
    [ ("send_logs_level", "none"); ("log_profile_events", "0"); ("compression", "0") ]
  in
  ...
  let test_program =
    ...
    printf "─────────────────────────────────────────────────────\n";
    let* result1 =
      DataTypeTests.test_bounded_query conn table_name "id, name, age, balance, bignum, created"
    in
    ...
    let conn2 =
      Connection.create ~host:"127.0.0.1" ~port:8463 ~compression:Compress.None ~settings ()
    in
      ...
      let insert_one id name =
        let insert =
          sprintf
            "INSERT INTO %s (id, name, age, balance, bignum, created) SELECT to_int32(%d), '%s', \
             to_uint32(%d), to_float64(%f), to_int64(%Ld), now()"
            table_name id name (30 + id)
            (2000.0 +. float_of_int id)
            (Int64.of_int (2000000 + id))
        in
        Client.execute client insert
      in
    ...
    let create_stream =
      sprintf
        "CREATE STREAM %s (id int32, value float64, message string, _tp_time datetime64(3) DEFAULT \
         now64(3), PRIMARY KEY (id)) SETTINGS mode='versioned_kv', version_column='_tp_time'"
        stream_table
    in
    ...
    let inserter_task =
      let rec loop count =
        if !stop_signal || count >= 10 then Lwt.return_unit
          ...
          let insert =
            sprintf "INSERT INTO %s (id, value, message) SELECT to_int32(%d), to_float64(%f), '%s'"
              stream_table id value message
          in
