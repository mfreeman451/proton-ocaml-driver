      ...
      let ops_result = Lwt_main.run (
        Lwt.catch
          (fun () -> 
            ...
            (* Create test table *)
            let* _ = Client.execute client 
              "CREATE STREAM IF NOT EXISTS test_auth_table (id int32, name string)" in
            Printf.printf "   ✅ Created test table\n";
            
            (* Insert test data *)
            let* _ = Client.execute client 
              "INSERT INTO test_auth_table (id, name) VALUES (1, 'test')" in
            Printf.printf "   ✅ Inserted test data\n";
            
            (* Query test data *)
            let* rows = Client.execute client 
              "SELECT id, name FROM test_auth_table ORDER BY id" in
...
let () =
  ...
  let tests = [
    ...
    ("localhost", 8465, Some "default", None, 
     "Testing default user connection");
