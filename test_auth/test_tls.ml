...
let test_tls_connection ~host ~port ?user ?password ~ca_cert ~client_cert ~client_key
    ~verify_hostname description =
  Printf.printf "\n🔒 %s\n" description;
  ...
  try
    ...
    let connectivity_success =
      match result with
      | None ->
          Printf.printf "   ❌ TLS connection test failed\n";
          false
      | Some rows ->
    ...
    let user_success =
      ...
      | Some expected_user -> (
          ...
          | Some (Client.Rows ([ row ], _)) ->
              let current_user =
                match row with [ value ] -> Column.value_to_string value | _ -> "unknown"
              in
...
let () =
  ...
  let results =
    List.map
      (fun (host, port, user, password, ca, cert, key, verify, desc) ->
        test_tls_connection ~host ~port ?user ?password ~ca_cert:ca ~client_cert:cert
          ~client_key:key ~verify_hostname:verify desc)
      tests
  in
  ...
  if successful > 0 then (
    ...
    List.iter2
      (fun (_, _, user, _, _, _, _, _, desc) success ->
    ...
    if successful = total then (
      ...
      exit 0)
    else exit 0 (* Some tests passed, that's still success *))
  else (
