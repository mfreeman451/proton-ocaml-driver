...
let run () =
  ...
  let program =
    ...
    let* _ =
      Lwt.catch
        ...
        (fun _ex -> ddl_mode_stream := true; Client.execute client drop_stream)
    in
    (if !ddl_mode_stream
     then printf "[insert_verify] Using STREAM dialect (dropped if existed)\n%!"
     else printf "[insert_verify] Using TABLE dialect (dropped if existed)\n%!");
    let* _ =
      if !ddl_mode_stream
      then Client.execute client create_stream
    ...
    let* _ =
      if !ddl_mode_stream then (
        let* _ = Client.execute client (sprintf "INSERT INTO %s (id, name) SELECT to_int32(1), 'one'" table) in
        let* _ = Client.execute client (sprintf "INSERT INTO %s (id, name) SELECT to_int32(2), 'two'" table) in
  ...
  let finalize () =
    ...
    let* _ = Lwt.catch (fun () -> Client.execute client drop_table) (fun _ -> Lwt.return Proton.Client.NoRows) in
    let* _ = Lwt.catch (fun () -> Client.execute client drop_stream) (fun _ -> Lwt.return Proton.Client.NoRows) in
