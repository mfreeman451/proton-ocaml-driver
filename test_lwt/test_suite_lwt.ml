...
let build_uncompressed_block ~cols =
  ...
  List.iter
    (fun (name, type_spec, rows) ->
      write_str buf name;
      write_str buf type_spec;
      let writeln_int32 v =
        Buffer.add_string buf
          (Bytes.to_string (Bytes.init 4 (fun i -> Char.chr ((v lsr (8 * i)) land 0xFF))))
      in
      let writeln_int64 v =
        Buffer.add_string buf
          (Bytes.to_string
      ...
      for _i = 1 to rows do
        ...
        | s when Column.has_prefix s "decimal(" ->
        ...
        | s when Column.has_prefix s "nullable(" ->
            Buffer.add_char buf '\x00';
            write_str buf "abc"
...
let test_lowcardinality_basic () =
  ...
  (* n_rows *)
  write_str buf "lc";
  write_str buf "LowCardinality(String)";
...
let test_lowcardinality_nullable () =
  ...
  write_varint buf 3;
  write_str buf "lcn";
  write_str buf "LowCardinality(Nullable(String))";
  Buffer.add_string buf "\x00\x00\x00\x00\x00\x00\x00\x00";
  (* key type *)
  Buffer.add_string buf "\x01\x00\x00\x00\x00\x00\x00\x00";
  (* dict rows=1 *)
  write_str buf "x";
  (* dict[0] = x *)
  Buffer.add_string buf "\x03\x00\x00\x00\x00\x00\x00\x00";
...
let test_compression_frame_format () =
  ...
  let () =
    ...
    let writev_fn ss =
      List.iter (fun s -> output_string oc s) ss;
      flush oc;
      Lwt.return_unit
    in
  ...
  Alcotest.(check int32)
    "Uncompressed size in header"
    (Int32.of_int (Bytes.length test_data))
    uncompressed_size
...
let test_checksum_verification () =
  ...
  let () =
    ...
    Alcotest.(check string)
      "Valid checksum verification" (Bytes.to_string test_data) (Bytes.to_string decompressed)
  in
...
let test_zstd_compression_ratio () =
  ...
  Printf.printf "ZSTD: Original size: %d, Compressed size: %d, Ratio: %.2f\n" original_size
    compressed_size
    (float_of_int original_size /. float_of_int compressed_size);
...
let test_zstd_frame_format () =
  ...
  Alcotest.(check string)
    "ZSTD frame format roundtrip" (Bytes.to_string test_data) (Bytes.to_string decompressed);
...
let test_connection_creation () =
  ...
  Alcotest.(check bool)
    "Connection created with correct defaults" true
...
let test_compression_enabled_client () =
  ...
  Alcotest.(check bool)
    "Client with None compression" true
    (client_none.conn.compression = Compress.None);
  Alcotest.(check bool)
    "Client with LZ4 compression" true
    (client_lz4.conn.compression = Compress.LZ4)

(* DateTime column tests *)
let test_datetime_parsing () =
  (* Test that parsing doesn't fail *)
  try
    let _reader = Column.reader_of_spec "datetime" in
    Alcotest.(check bool) "DateTime parser created" true true
  with _ -> Alcotest.(check bool) "DateTime parser created" true false

let test_datetime_with_timezone () =
  try
    let _reader = Column.reader_of_spec "datetime('UTC')" in
    Alcotest.(check bool) "DateTime with timezone parser created" true true
  with _ -> Alcotest.(check bool) "DateTime with timezone parser created" true false

let test_datetime64_parsing () =
  try
    let _reader = Column.reader_of_spec "datetime64(3)" in
    Alcotest.(check bool) "DateTime64 parser created" true true
  with _ -> Alcotest.(check bool) "DateTime64 parser created" true false

let test_datetime64_with_timezone () =
  try
    let _reader = Column.reader_of_spec "datetime64(6, 'America/New_York')" in
    Alcotest.(check bool) "DateTime64 with timezone parser created" true true
  with _ -> Alcotest.(check bool) "DateTime64 with timezone parser created" true false

let test_datetime_value_formatting () =
  let dt_value = Column.DateTime (1609459200L, Some "UTC") in
...
let test_datetime_binary_roundtrip () =
  ...
  let reader = Column.reader_of_spec_br "datetime" in
...
let test_datetime64_binary_roundtrip () =
  ...
  let test_value = 1609459200123L in
  ...
  let reader = Column.reader_of_spec_br "datetime64(3)" in
  let values = reader br 1 in
...
let test_multiple_datetime_values () =
  ...
  let timestamps = [ 1609459200L; 1640995200L; 1672531200L ] in
  (* 2021, 2022, 2023 *)
  let bytes_data = Bytes.create 12 in
  (* 3 * 4 bytes *)
...
let test_array_parsing () =
  try
    let _reader = Column.reader_of_spec "array(string)" in
    Alcotest.(check bool) "Array parser created" true true
  with _ -> Alcotest.(check bool) "Array parser created" true false

let test_nested_array_parsing () =
  try
    let _reader = Column.reader_of_spec "array(array(int32))" in
    Alcotest.(check bool) "Nested array parser created" true true
  with _ -> Alcotest.(check bool) "Nested array parser created" true false

let test_array_value_formatting () =
  let array_value =
    Column.Array [| Column.String "hello"; Column.String "world"; Column.Int32 42l |]
  in
...
let test_map_parsing () =
  try
    let _reader = Column.reader_of_spec "map(string, int32)" in
    Alcotest.(check bool) "Map parser created" true true
  with _ -> Alcotest.(check bool) "Map parser created" true false

let test_nested_map_parsing () =
  try
    let _reader = Column.reader_of_spec "map(string, array(int32))" in
    Alcotest.(check bool) "Nested map parser created" true true
  with _ -> Alcotest.(check bool) "Nested map parser created" true false

let test_map_value_formatting () =
  let map_value =
    Column.Map
      [ (Column.String "key1", Column.Int32 42l); (Column.String "key2", Column.String "value2") ]
  in
...
let test_enum8_parsing () =
  ...
  | { Block.n_rows = 3; columns = [| c |]; _ } -> (
      Alcotest.(check string)
        "Enum8 type" "Enum8('active'=1,'inactive'=2,'pending'=3)" c.Block.type_spec;
...
let test_enum16_parsing () =
  ...
  | { Block.n_rows = 2; columns = [| c |]; _ } -> (
      Alcotest.(check string)
        "Enum16 type" "Enum16('low'=100,'normal'=200,'high'=300)" c.Block.type_spec;
      match c.Block.data with
...
let test_batch_size_limits () =
  let conn = Connection.create ~compression:Compress.None () in
  let config =
    { (Async_insert.default_config "test_table") with max_batch_size = 2; max_batch_bytes = 1000 }
  in
  let inserter = Async_insert.create config conn in

  let test_rows =
    [ [ Column.String "test1"; Column.Int32 1l ]; [ Column.String "test2"; Column.Int32 2l ] ]
  in
  let columns = [ ("name", "String"); ("value", "Int32") ] in
    ...
    [
      ...
      ( "Connection",
        [
          ...
          Alcotest.test_case "Compression-enabled connection" `Quick
            test_compression_enabled_connection;
