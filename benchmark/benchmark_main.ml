...
(* TLS configuration *)
let use_tls =
  match Sys.getenv_opt "PROTON_TLS" with
...
let tls_config =
  if use_tls then
    ...
    let verify_hostname =
      match Sys.getenv_opt "PROTON_VERIFY_HOSTNAME" with
...
let print_connection_info () =
  ...
  let auth_status =
    match user with Some u -> sprintf "User: %s" u | None -> "No authentication"
  in
...
let bench_reader_for_spec ~spec ~rows ~loops make_bytes =
  ...
  Column.reset_cache_stats ();
  Column.clear_reader_caches ();
  let _, warm_s =
    time (fun () ->
        for _i = 1 to loops do
          let br = Buffered_reader.create_from_bytes_no_copy payload in
...
let verify_count client table_name ~expected =
  ...
  let rec loop attempts =
    let query =
      sprintf "SELECT to_uint64(count()) FROM %s SETTINGS query_mode='table'" table_name
    in
    let* res =
      Lwt.catch
...
let bench_async_batch_insert ?(compression = Compress.None) count =
  ...
  let* _, elapsed =
    time_it_lwt (sprintf "ASYNC_BATCH_INSERT_%d_ROWS" count) (fun () ->
...
let bench_manual_async_insert ?(compression = Compress.None) count batch_size =
  ...
  let config =
    {
      (Async_insert.default_config table_name) with
  ...
  let* _, elapsed =
    time_it_lwt (sprintf "MANUAL_ASYNC_%d_BATCH_%d" count batch_size) (fun () ->
  ...
  printf "manual-async: %7d rows (batch=%4d) | %6.2fs | %8.0f rows/s | verified=%b\n%!" count
    batch_size elapsed rate (verified = count);
...
let bench_live_streaming_read ?(compression = Compress.None) count =
  ...
  let* rows_read, elapsed =
    time_it_lwt (sprintf "LIVE_STREAM_READ_%d_ROWS" count) (fun () ->
...
let bench_compression_insert ?(compression = Compress.None) count =
  ...
  let* _, elapsed =
    time_it_lwt (sprintf "COMPRESSION_BATCH_INSERT_%d_ROWS" count) (fun () ->
        (* Use the high-level async batch insert from Client *)
        Client.insert_rows ~columns client table_name rows)
  in
...
let bench_compression_insert_float ?(compression = Compress.None) count =
  ...
  let* _, elapsed =
    time_it_lwt (sprintf "COMPRESSION_BATCH_INSERT_F64_%d_ROWS" count) (fun () ->
        Client.insert_rows ~columns client table_name rows)
  in
...
let run_compression_benchmarks () =
  ...
  let* () =
    if truthy "SKIP_COMP_CONNECTIVITY" then Lwt.return_unit
    else (
  ...
  let test_rows =
    match Sys.getenv_opt "COMP_ROWS" with
    | Some s -> int_of_string_opt (String.trim s) |> Option.value ~default:10000
    | None -> 10000
  in
...
let run_insert_benchmarks sizes =
  ...
  List.iter (fun (size, rate) -> printf "  %7d rows: %10.0f rows/sec\n%!" size rate) async_rates;

  printf "\nManual Async Insert Performance (10K rows):\n%!";
  List.iter
    (fun (batch_size, rate) -> printf "  batch %4d: %10.0f rows/sec\n%!" batch_size rate)
    manual_rates;
...
let test_million_rows () =
  ...
  printf "  Uncompressed: %.0f rows/sec (%.2f minutes)\n%!" baseline_rate
    (1_000_000.0 /. baseline_rate /. 60.0);
  printf "  LZ4 Compressed: %.0f rows/sec (%.2f minutes)\n%!" lz4_rate
    (1_000_000.0 /. lz4_rate /. 60.0);
...
let run_benchmarks () =
  ...
  let only_reader =
    match Sys.getenv_opt "ONLY_READER_MICRO" with
    | Some ("1" | "true" | "TRUE" | "yes" | "YES") -> true
    | _ -> false
  in
  if only_reader then (
    ...
    let* () =
      if truthy "ONLY_COMPRESSION" then (
        ...
        let* () =
          if truthy "SKIP_INSERT" then (
            printf "Skipping insert benchmarks.\n%!";
            Lwt.return_unit)
          else run_insert_benchmarks test_sizes
        in
        let* () =
          if truthy "SKIP_STREAMING" then (
            printf "Skipping streaming benchmarks.\n%!";
            Lwt.return_unit)
          else run_streaming_benchmarks streaming_sizes
        in
        let* () =
          if truthy "SKIP_COMPRESSION" then (
            printf "Skipping compression benchmarks.\n%!";
            Lwt.return_unit)
          else run_compression_benchmarks ()
        in
        let* () =
          if truthy "SKIP_MILLION" then (
            printf "Skipping 1M row challenge.\n%!";
            Lwt.return_unit)
          else test_million_rows ()
        in
        Lwt.return_unit
    in
